...
factor = 1.5;
Rinv = factor*Rinv/sum(sum(Rinv)); % normalized R inverse, so that the sample is still within the voxel world
...
while abs(Qtheta - QthetaOld) > convergenceThreshold
    ...
    if iter > 100 || sum(dtheta_smoothed,'all') == 0
        disp('Maximum iteration (100) has reached.')
        break
    end
    ...
    [Rinv, factor] = computeAdaptativeConvarianceMatrix(Rinv, Qtheta, QthetaOld, factor);
