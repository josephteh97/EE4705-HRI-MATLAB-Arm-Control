...
box_center = [0.6 0.0 0.35]; % (metric) world coordinates of the box center
aObs = collisionBox(lbox,lbox,lbox);  
aObs.Pose = trvec2tform(box_center);   

lbox2 = 0.08*2; % length of the cube
box_center2 = [0.6 0.0 0.1]; % (metric) world coordinates of the box center
aObs2 = collisionBox(lbox2,lbox2,lbox2);  
aObs2.Pose = trvec2tform(box_center2);   
...
%% voxelize the box obstacles
cube_metric2 = [box_center2-lbox2;
                lbox2,lbox2,lbox2]; % [xmin, ymin, zmin] for 1st row, xyz-lengths for 2nd row
% range (lower and upper limits) of the cube voxel
% The number inside ceil is always positive
cube_voxel2 = [ceil((cube_metric2(1, :)-Env_size(1,:))./voxel_size); ...
    ceil((cube_metric2(1, :) + cube_metric2(2, :)-Env_size(1,:))./voxel_size)];

% Update the cube occupancy in the voxel world
% First generate the 3D grid coordinates (x,y,z subcripts) of cube in the voxel world
[xc2, yc2, zc2] = meshgrid(cube_voxel2(1, 1):cube_voxel2(2, 1), cube_voxel2(1, 2):cube_voxel2(2, 2), cube_voxel2(1, 3):cube_voxel2(2, 3));
% Set the corresponding voxel occupancy to 1, meaning occupied
binary_world(sub2ind([Env_size(2, 1) / voxel_size(1), Env_size(2, 2) / voxel_size(2), Env_size(2, 3) / voxel_size(3)], xc2, yc2, zc2)) = 1;
